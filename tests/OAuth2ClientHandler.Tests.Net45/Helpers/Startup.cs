...
namespace OAuth2ClientHandler.Tests.Net45
{
    internal sealed class Startup
    {
        public void Configuration(IAppBuilder app)
        {
            app.UseOAuthAuthorizationServer(new OAuthAuthorizationServerOptions
            {
                ...
                Provider = new OAuthAuthorizationServerProvider
                {
                    ...
                    {
                        if (HasValidBasicCredentials(ctx, out var clientId)) ctx.Validated(clientId);
        ...
        private static bool HasValidBasicCredentials(OAuthValidateClientAuthenticationContext ctx, out string clientId)
            => ctx.TryGetBasicCredentials(out clientId, out var clientSecret)
        ...
        private static bool HasValidFormCredentials(OAuthValidateClientAuthenticationContext ctx, out string clientId)
            => ctx.TryGetFormCredentials(out clientId, out var clientSecret)
